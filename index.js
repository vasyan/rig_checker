...
const makeRequest = (uri, callback) => request(uri, { json: true }, callback);
...
bot.command("debug", async ctx => {
  ...
  makeRequest(API_URI + user.wallet, async (err, res, body) => {
    if (err) {
...
bot.command("clearall", async ctx => {
  ModelUser.findByIdAndUpdate(
    ...
    err => {
      if (err) {
        throw new Error("Error on clear all alarms");
      }

      telegram.sendMessage(ctx.chat.id, `Alarms has been cleared`);
    }
  );
});
...
bot.hears(/^\/clear (.*)/, async ctx => {
  ...
  const [, worker] = ctx.match;
  const user = await ModelUser.findById(ctx.chat.id);

  if (user.alarms.indexOf(worker) === -1) {
    ctx.reply("nope");
    return;
  }
  ...
  ModelUser.findByIdAndUpdate(
    ...
    err => {
      if (err) {
        throw new Error(`Error on remove alarms ${worker}`);
      }
      telegram.sendMessage(ctx.chat.id, `Alarm ${worker} has been removed`);
    }
  );
});
...
async function pong() {
  ...
  R.forEach(user => {
    telegram
      ...
      .catch(err => {
        console.warn("--- Pong processing --- Sorry, chat not found", user._id);
      });
  }, users);
}
